...
const ImageCropper = () => {
    ...
    const handleAutoCrop = () => {
        if (cropper.current) {
            cropper.current.setCropBoxData({ top: 0, height: cropper.current.getCanvasData().height });
    ...
    function orderItems() {
        ...
        pckryRef.current.getItemElements().forEach((itemElem, i) => {
            ...
            const aspectRatioStyle = itemElem.getAttribute('style').match(/aspect-ratio:\s*(\d+)\s*\/\s*(\d+)/);
    ...
    const updateMasonry = async () => {
        const itemOrderArray = [];
        await Promise.all(pckryRef.current.getItemElements().map(async (itemElem, i) => {
            const imgSrc = itemElem.querySelector('img').getAttribute('src');
            const idImg = itemElem.getAttribute('id');
            const widthClass = itemElem.classList[itemElem.classList.length - 1];
            const aspectRatioStyle = itemElem.getAttribute('style').match(/aspect-ratio:\s*(\d+)\s*\/\s*(\d+)/);
            ...
            if (imgSrc.startsWith('data:image')) {
                try {
                    const data = await fileApi.fileBase64({ base64: imgSrc, folder: location.pathname.split('/').pop() });
        ...
        masonryPageApi.updateMasonry(location.pathname.split('/').pop(), { img: itemOrderArray })
            .then(() => {
            ...
            .catch((error) => {
                if (error.response.status === 404) {
                    masonryPageApi.addMasonry(location.pathname.split('/').pop())
    ...
    useEffect(() => {
        if (croppedImages.length > 0 && croppedImages.length === images.length) {
            const items = croppedImages.map((image, index) => createItemAndBindEvents(image, aspectRatios[index], aspectRatios[index][0]));
            items.forEach((item) => {
        ...
        <>
            <div className="!h-full">
                <div className="flex justify-center items-center gap-2 my-2">
                    ...
                    <input type="file" hidden multiple onChange={handleFileChange} ref={inputFileRef} />
                        ...
                        images.length > 0 && (
                            <>
                                ...
                                <Modal
                                    title={
                                        <div className='flex justify-between items-center mb-3'>
                                            ...
                                            <Button onClick={handleAutoCrop}>
                                                <i className="fa-duotone fa-crop-simple fs-5 me-2"></i>Tự động cắt ảnh
                                            </Button>
                                        </div>
                                    }
                                    centered
                                    ...
                                    footer={[
                                        <Button key="exit" danger onClick={() => setOpenModal(false)}> Thoát </Button>,
                                        <Button key="back" type="dashed" onClick={handleCropBack} disabled={currentImageIndex === 0}>Trở về</Button>,
                                        <Button key="next" type="dashed" onClick={handleCropNext} disabled={lastImage || images.length === 1}>Tiếp theo</Button>,
                                        <Button key="upload" type="primary" onClick={handleCropUpload} disabled={!(lastImage || images.length === 1)}> Tải lên</Button>
                                    ]}
                                >
                ...
                <div style={{ height: 600, overflow: 'scroll', paddingBottom: 50 }}>
                    <div className="grid" ref={gridRef}>
                        ...
                        {gridItems?.map((item, index) => (
                            ...
                            >
                                <img className='relative' src={item.imgSrc} onContextMenu={(event) => handleRightClick(event, item)} />

                                <Input className='bg-dark text-white absolute bottom-0 end-0'
                                    style={{ width: '100%' }}
                                    value={item?.link}
                                    id={item?.idImg}
            ...
            <Menu id="menu">
                <Submenu label={`Thay đổi kích thước`}>
                    <Item onClick={() => {
                        handleSizeChange(selectedImg.aspectRatio, 1, selectedImg, gridItems, setGridItems, pckryRef);
                        setTimeout(() => {
                            if (isDraggable) {
                                toggleDraggable();
                            }
                    ...
                    <Item onClick={() => {
                        handleSizeChange(selectedImg.aspectRatio, 2, selectedImg, gridItems, setGridItems, pckryRef);
                        setTimeout(() => {
                            if (isDraggable) {
                                toggleDraggable();
                            }
                    ...
                    <Item onClick={() => {
                        handleSizeChange(selectedImg.aspectRatio, 3, selectedImg, gridItems, setGridItems, pckryRef);
                        setTimeout(() => {
                            if (isDraggable) {
                                toggleDraggable();
                            }
