...
const Page = ({ data, canvas, pluginss, plugins, pluginsOpts, savePage, slug, folder, configGrapeJs, height = '100vh' }) => {
    ...
    const save = (editor) => {
        ...
        editor.Commands.add('save-page', {
            run(editor, sender) {
                var html = editor.getHtml().replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '');
        ...
        <>
            ...
            <GjsEditor
                ...
                options={{
                    ...
                    assetManager: {
                        uploadFile: (ev) => {
                            toastLoading('file', 'Kiên nhẫn đang tải file và resize file', 'Các tấm ảnh có width > 1300 sẽ được tối ưu hóa!');
                            ...
                            fileApi.uploadFile(formData)
                                ...
                                .catch(() => {
                                    toastError('file', 'Tải lên file thất bại', 'Tấm ảnh không được tối ưu hóa!');
                                });
                        }
                    },
                    styleManager: {
