...
const EditableCell = React.memo(({ editing, dataIndex, title, type, optionSelect, children, ...restProps }) => {
    const inputNode = useMemo(() => {
        switch (type) {
            ...
            case 'percent':
                return <InputNumber className='w-full' min={0} max={100} placeholder={title} />;
            case 'select-multi':
                return <Select allowClear mode="multiple" placeholder={title} options={optionSelect} />;
            case 'select':
...
const EditableTable = ({ Api, ApiPut, total, dragMode = true, loading, data, columns, onSave, onDelete, button, colEdit = true, isScroll = true, scroll, width }) => {
    ...
    const save = useCallback(async () => {
        try {
            ...
            toastError(errInfo, 'Lưu thay đổi thất bại !', 'Vui lòng kiểm tra lại thông tin !');
        }
    ...
    const onDragEnd = useCallback(({ active, over }) => {
        if (active.id !== over?.id) {
            ...
            const filteredDataSource = updatedDataSource.map(({ _id, order }) => ({ _id, order }));
            dispatch(ApiPut(filteredDataSource));
            toastSuccess('drag', 'Thay đổi vị trí thành công !', 'Dữ liệu đã được cập nhật !');
        }
    }, [data]);
    ...
    const handleChange = useCallback((page, pageSize) => {
        cancel();
        dispatch(Api({ page, limit: pageSize }));
        setCurrent(page);
    }, [dispatch, Api, cancel]);

    const handleShowSizeChange = useCallback((current, size) => {
        dispatch(Api({ page: 1, limit: size }));
        setPageSize(size);
                ...
                <DndContext sensors={sensors} modifiers={[restrictToVerticalAxis]} onDragEnd={onDragEnd}>
                    <SortableContext items={data.map((i) => i.key)} strategy={verticalListSortingStrategy}>
                        <Table
                                    ...
                                    render: (_, record) => {
                                        ...
                                        return editable ? (
                                            <span className='flex justify-center gap-3' style={{ cursor: 'pointer' }}>
                                                ...
                                                <Popconfirm placement="bottomRight" title="Bạn có chắc muốn lưu thay đổi" onConfirm={save}>
                                            ...
                                            <div className="flex justify-center items-center gap-3" style={{ cursor: 'pointer' }}>
                                                ...
                                                <Popconfirm disabled={editingKey !== ''} placement="bottomRight" title="Bạn có chắc muốn xóa !" onConfirm={() => onDelete(record._id)}>
                            ...
                            scroll={isScroll ? (scroll || { x: 1200, y: 'calc(100vh - 255px)' }) : undefined}
